    ...
    if (type === "ton") {
      try {
        const response = await fetch(
          `https://tonapi.io/v2/accounts/${user.address}`
        );
    ...
    <>
      <div className="relative ">
        <input
          ...
          value={amount}
          placeholder="10 TON"
          className="AmountComponent-inputs-style"
          type="number"
        />
        <p className="text-[#3bffff] text-[15px] font-light absolute top-[15px] left-5">
          DTS AMOUNT:
        </p>
      </div>
      <div className="flex mt-1">
        <p className="text-white ms-4">USD AMOUNT :</p>
        <p className="text-[#3bffff] ms-1">{(amount * 0.01).toFixed(2)} $</p>
      </div>
    </>
  );
}
